...
<div class="mx-auto max-w-[64rem]">
    <div class="mb-8">
        <h2 class="text-[2rem] font-semibold tracking-tight text-gray-900 dark:text-white">
            Supervisor Dashboard
        </h2>
        <p class="mt-2 max-w-2xl text-sm leading-6 text-slate-500 dark:text-slate-400">
            System health, recent activity, and operator signals for the current domain.
        </p>
    </div>
    ...
    {:else if data}
            ...
            <div class="space-y-3">
                {#each data.alerts as alert}
                    <ErrorBanner
                        ...
                        class="shadow-sm"
                        title={alert.type === "critical" ? "Critical Alert" : "Attention Needed"}
                    />
                {/each}
            </div>
        {/if}
        ...
        <div class="grid grid-cols-1 gap-5 lg:grid-cols-3">
            <Surface class="flex flex-row items-center justify-between px-5 py-4">
                <div>
                ...
                <Surface class="flex flex-row items-center justify-between px-5 py-4">
                    <div>
                        ...
                        <p class="mt-1 text-3xl font-bold text-emerald-600 dark:text-emerald-400">
                            ...
                            <span class="text-lg font-medium text-slate-500 dark:text-slate-400">Sats</span>
                        </p>
                    </div>
                    <div
            ...
            <div class="grid grid-cols-1 gap-5 lg:grid-cols-3">
                <Surface class="px-5 py-4">
                    <div class="flex items-start justify-between gap-4">
                        <div>
                            <h4 class="text-sm font-medium uppercase tracking-wide text-slate-500 dark:text-slate-400">
                                Worker
                            </h4>
                            <p
                        ...
                        <Badge
                            variant={data.agentRunSummary.worker.lastError
                                ? "danger"
                                : data.agentRunSummary.worker.started
                                  ? "success"
                                  : "muted"}
                    ...
                    >
                        <div>
                            <dt class="text-xs uppercase tracking-wide text-slate-400 dark:text-slate-500">
                                Interval
                            </dt>
                            <dd>{Math.round(
                ...
                <Surface class="px-5 py-4">
                    ...
                    <div class="mt-4 space-y-3">
                        <div class="flex items-center justify-between text-sm">
                            <span class="text-slate-500 dark:text-slate-400">Waiting approval</span>
                            <span class="font-medium text-gray-900 dark:text-white">
                                {data.agentRunSummary.queue.waitingApproval}
                            </span>
                        </div>
                        <div class="flex items-center justify-between text-sm">
                            <span class="text-slate-500 dark:text-slate-400">Currently running</span>
                            <span class="font-medium text-gray-900 dark:text-white">
                                {data.agentRunSummary.queue.running}
                            </span>
                        </div>
                        <div class="flex items-center justify-between text-sm">
                            <span class="text-slate-500 dark:text-slate-400">24h completed</span>
                            <span class="font-medium text-gray-900 dark:text-white">
                ...
                <Surface class="px-5 py-4">
                    ...
                    <div class="mt-4 space-y-3">
                        <div class="flex items-center justify-between text-sm">
                            <span class="text-slate-500 dark:text-slate-400">Review steps succeeded</span>
                            <span class="font-medium text-gray-900 dark:text-white">
                                {data.agentRunSummary.throughput.reviewActionsSucceeded}
                            </span>
                        </div>
                        <div class="flex items-center justify-between text-sm">
                            <span class="text-slate-500 dark:text-slate-400">Quality checks succeeded</span>
                            <span class="font-medium text-gray-900 dark:text-white">
                                {data.agentRunSummary.throughput.qualityChecksSucceeded}
                            </span>
                        </div>
                        <div class="flex items-center justify-between text-sm">
                            <span class="text-slate-500 dark:text-slate-400">Settled failures</span>
                            <span class="font-medium text-gray-900 dark:text-white">
                                {data.agentRunSummary.failures.settledFailed}
                            </span>
                        </div>
                        <div class="flex items-center justify-between text-sm">
                            <span class="text-slate-500 dark:text-slate-400">Policy denials</span>
                            <span class="font-medium text-gray-900 dark:text-white">
        ...
        >
            <Surface class="flex flex-col items-center justify-center px-4 py-3 text-center">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
                    <thead class="bg-slate-50/80 dark:bg-slate-900/60">
                        <tr>
                            <th class="px-6 py-3 text-left text-[0.72rem] font-semibold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">Timestamp</th>
                            <th class="px-6 py-3 text-left text-[0.72rem] font-semibold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">Action</th>
                            <th class="px-6 py-3 text-left text-[0.72rem] font-semibold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">Entity</th>
                            <th class="px-6 py-3 text-left text-[0.72rem] font-semibold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">Actor</th>
                        </tr>
                    </thead>
