...
*   **Test-Driven Validation:** We rely on live verification commands (for example `npm run verify:api` and `npm run verify:dry-run`) locally before considering a feature "done".
*   **Atomic Commits:** Commits pushed to the remote repository should be granular, mapping clearly to singular functional changes. We avoid massive "kitchen sink" commits where possible.
*   **Commit Messaging:** We strive for descriptive, conventional commit messages: `feat(api): ...`, `fix(ui): ...`, `docs: ...` identifying exactly what was altered in the commit subject. Autonomous agents **MUST** read the [Agent Commit Standards](../guides/agent-commit-standards.md) prior to dispatching any commit commands to ensure the Semantic Release action successfully builds the changelog.
*   **Git Identity:** All automated commits generated by the AI assistant are authored formally under the `WordClaw Agent <agent@wordclaw.dev>` identity.
*   **Continuous Synchronization:** We heavily prioritize fast `add`, `commit`, and `push` cycles, sending small robust packets of code upstream consistently to ensure the remote tracking remains in parity with local state.
