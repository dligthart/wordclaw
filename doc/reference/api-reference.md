...
For deployment-level discovery before authentication, use `GET /api/capabilities`. It reports the current protocol contract, enabled modules, auth/domain expectations, dry-run coverage, and task-oriented agent recipes in one machine-readable manifest.
